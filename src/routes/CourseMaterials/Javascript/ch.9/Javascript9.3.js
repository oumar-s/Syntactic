...
const Examination9 = () => {
	...
	const handleMorePractice = async () => {
		const newProblems = await generatePracticeProblems(
			'just about promises in regards to then, catch, async and await',
			practiceProblems,
			setIsLoading,
		);
	...
	const setProgress = async () => {
		...
		if (currentUser) {
			...
			if (!data.Javascript['9:3']) {
				await updateDoc(progressRef, {
					'Javascript.9:3': 'complete',
	...
	return (
		<div className='bg-midnight pt-1'>
			<div className='flex justify-center space-x-8 p-5 m-5'>
				...
				<Link to='/javascript/projects' onClick={setProgress}>
					<img
						className='bg-slate-400 hover:bg-gray-300 p-2'
						src={rightArrowIcon}
