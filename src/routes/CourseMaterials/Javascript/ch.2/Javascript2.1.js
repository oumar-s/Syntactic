...
const IntroConditionalStatements = () => {
    ...
    const handleSubmit1 = async () => {
        ...
        if (currentUser) {
            ...
            if (docSnap.exists()) {
                updateDoc(docRef, {
                    feedbacks: arrayUnion({ feedback: feedback, course: 'Javascript', problem:  Chapter1.topic1.practice1})
                });
            } else {
                
                await setDoc(docRef, {
                    feedbacks: [{course: 'Javascript', feedback: feedback, problem: Chapter1.topic1.practice1}]
                });
            }
        };
    ...
    const handleSubmit2 = async () => {
        ...
        if (currentUser) {
            ...
            if (docSnap.exists()) {
                updateDoc(docRef, {
                    feedbacks: arrayUnion({ feedback: feedback, course: 'Javascript', problem:  Chapter1.topic1.practice2})
                });
            } else {
                
                await setDoc(docRef, {
                    feedbacks: [{course: 'Javascript', feedback: feedback, problem: Chapter1.topic1.practice2}]
                });
            }
    ...
    const handleSubmit3 = async () => {
        ...
        if (currentUser) {
            ...
            if (docSnap.exists()) {
                updateDoc(docRef, {
                    feedbacks: arrayUnion({ feedback: feedback, course: 'Javascript', problem:  Chapter1.topic1.practice3})
                });
            } else {
                
                await setDoc(docRef, {
                    feedbacks: [{course: 'Javascript', feedback: feedback, problem: Chapter1.topic1.practice3}]
                });
            }
	...
	const setProgress = async () => {
		...
		if (currentUser) {
			...
			if (!data.Javascript['2:1']) {
				await updateDoc(progressRef, {
					'Javascript.2:1': 'complete',
		...
		<div className='flex flex-col font-ubuntu bg-midnight text-white'>
			<div className='justify-items-center ml-20 mr-20 pl-20 pr-20'>
				<div className='flex justify-center space-x-8 p-5 m-5'>
					...
					<Link to='/javascript/2.2' onClick={setProgress}>
						<img
							className='bg-slate-400 hover:bg-gray-300 p-2'
							src={rightArrowIcon}
				...
				<p className='font-mono my-8'>
					Conditional statements form the core of logic in programming. In this
					topic, we'll introduce you to the fundamental concept of conditional
					statements, focusing on two essential constructs: ‘if’ statements and
					‘else’ statements. By the end of this topic, you'll understand how to
					use these statements effectively in your JavaScript programs.
				</p>

				<p className='font-mono my-8'>
					Conditional statements are like the traffic signals of your program,
					guiding the flow of execution based on certain conditions. We'll
					cover:
					<br></br>if Statements: Learn how to use ‘if’ statements to execute a
					block of code if a specified condition is true.
					<br></br>else Statements: Explore ‘else’ statements, which allow you
					to provide an alternative code block to execute if the condition in
				...
				<p className='font-mono mb-8'>
					Simple number comparison. In this example we're comparing two numbers
				...
				<div className='mt-10'>
					...
					<p className='font-mono my-4'>
						Write an if statement to check if a number is even or odd. Declare a
						variable called number and assign it any integer value. Write an if
						statement that checks whether the number is even or odd. Use
						console.log() to display a message indicating whether the number is
						even or odd.
					</p>
				...
				<div className='mt-10'>
					...
					<p className='font-mono my-4'>
						Now, let's apply conditional statements to create a basic login
						system. Declare a variable called isLoggedIn and set it to true or
						false to represent a user's login status. Write an if-else statement
						to check whether the user is logged in or not. Display different
						messages depending on the user's login status.
					</p>
				...
				<div className='mt-10'>
					...
					<p className='font-mono my-4'>
						In this practice, you'll simulate an online store's discount system.
						Create variables to represent a user's total purchase amount and
						their membership status (you can use a boolean value to represent
						membership). Write conditional statements to determine the discount
						percentage based on the purchase amount and membership status.
...
export default IntroConditionalStatements;
