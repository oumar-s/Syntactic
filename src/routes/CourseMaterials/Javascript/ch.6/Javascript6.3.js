...
const StringMethods = () => {
    ...
    const handleSubmit1 = async () => {
        ...
        if (currentUser) {
            ...
            if (docSnap.exists()) {
                updateDoc(docRef, {
                    feedbacks: arrayUnion({ feedback: feedback, course: 'Javascript', problem:  Chapter1.topic3.practice1})
                });
            } else {
                
                await setDoc(docRef, {
                    feedbacks: [{course: 'Javascript', feedback: feedback, problem: Chapter1.topic3.practice1}]
                });
            }
        };
    ...
    const handleSubmit2 = async () => {
        ...
        if (currentUser) {
            ...
            if (docSnap.exists()) {
                updateDoc(docRef, {
                    feedbacks: arrayUnion({ feedback: feedback, course: 'Javascript', problem:  Chapter1.topic3.practice2})
                });
            } else {
                
                await setDoc(docRef, {
                    feedbacks: [{course: 'Javascript', feedback: feedback, problem: Chapter1.topic3.practice2}]
                });
            }
    ...
    const handleSubmit3 = async () => {
        ...
        if (currentUser) {
            ...
            if (docSnap.exists()) {
                updateDoc(docRef, {
                    feedbacks: arrayUnion({ feedback: feedback, course: 'Javascript', problem:  Chapter1.topic3.practice3})
                });
            } else {
                
                await setDoc(docRef, {
                    feedbacks: [{course: 'Javascript', feedback: feedback, problem: Chapter1.topic3.practice3}]
                });
            }
	...
	const setProgress = async () => {
		...
		if (currentUser) {
			...
			if (!data.Javascript['6:3']) {
				await updateDoc(progressRef, {
					'Javascript.6:3': 'complete',
		...
		<div className='flex flex-col font-ubuntu bg-midnight text-white'>
			<div className='justify-items-center ml-20 mr-20 pl-20 pr-20'>
				<div className='flex justify-center space-x-8 p-5 m-5'>
					...
					<Link to='/javascript/6.4' onClick={setProgress}>
						<img
							className='bg-slate-400 hover:bg-gray-300 p-2'
							src={rightArrowIcon}
				...
				<p className='font-mono my-8'>toUpperCase</p>

				<p className='font-mono my-8'>
					In JavaScript, the toUpperCase() method is a built-in method for
					strings that is used to convert all the characters in a string to
					uppercase letters. This method doesn't modify the original string but
				...
				<div className='mt-10'>
					...
					<p className='font-mono my-4'>
						Write a JavaScript program that converts your name to uppercase
						using the toUpperCase() method. Then, display a message that says,
				...
				<p className='font-mono my-8'>Slice</p>

				<p className='font-mono my-8'>
					The slice() method in JavaScript is used to extract a portion of a
					string and create a new string from that portion. It takes one or two
					arguments: the start index and optionally the end index (exclusive).
					It does not modify the original string but returns a new string with
					the extracted characters. Here's how to use it with an example:
				</p>
				...
				<div className='mt-10'>
					...
					<p className='font-mono my-4'>
						Write a JavaScript program that takes an email address as a string
						and uses the slice() method to extract and display the domain part
						of the email address. Email:"john.doe@example.com."
					</p>
				...
				<p className='font-mono my-8'>Split</p>

				<p className='font-mono my-8'>
					The split() method in JavaScript is used to split a string into an
					array of substrings based on a specified separator.
				</p>
				...
				<div className='mt-10'>
					...
					<p className='font-mono my-4'>
						Create a program that takes a comma-separated values (CSV) string as
						input and splits it into an array of elements using the split()
						method. <br></br>csvData:"John,Doe,30,New York";:
					</p>
