...
const ObjectMethodValues = () => {
    ...
    const handleSubmit1 = async () => {
        ...
        try {
            const response = await fetch('http://127.0.0.1:5000/api/submitcode', {
              ...
              body: JSON.stringify({ code: code1, practice: Chapter1.topic4.practice1 }),
            });
        ...
        if (currentUser) {
            ...
            if (docSnap.exists()) {
                updateDoc(docRef, {
                    feedbacks: arrayUnion({ feedback: feedback, course: 'Javascript', problem:  Chapter1.topic4.practice1})
                });
            } else {
                // Add a new document in collection "cities"
                await setDoc(docRef, {
                    feedbacks: [{course: 'Javascript', feedback: feedback, problem: Chapter1.topic4.practice1}]
                });
            }
        };
    ...
    const handleSubmit2 = async () => {
        ...
        try {
            const response = await fetch('http://127.0.0.1:5000/api/submitcode', {
              ...
              body: JSON.stringify({ code: code2, practice: Chapter1.topic4.practice2 }),
            });
        ...
        if (currentUser) {
            ...
            if (docSnap.exists()) {
                updateDoc(docRef, {
                    feedbacks: arrayUnion({ feedback: feedback, course: 'Javascript', problem:  Chapter1.topic4.practice2})
                });
            } else {
                // Add a new document in collection "cities"
                await setDoc(docRef, {
                    feedbacks: [{course: 'Javascript', feedback: feedback, problem: Chapter1.topic4.practice2}]
                });
            }
    ...
    const handleSubmit3 = async () => {
        ...
        try {
            const response = await fetch('http://127.0.0.1:5000/api/submitcode', {
              ...
              body: JSON.stringify({ code: code1, practice: Chapter1.topic4.practice3}),
            });
        ...
        if (currentUser) {
            ...
            if (docSnap.exists()) {
                updateDoc(docRef, {
                    feedbacks: arrayUnion({ feedback: feedback, course: 'Javascript', problem:  Chapter1.topic4.practice3})
                });
            } else {
                // Add a new document in collection "cities"
                await setDoc(docRef, {
                    feedbacks: [{course: 'Javascript', feedback: feedback, problem: Chapter1.topic4.practice3}]
                });
            }
	...
	const setProgress = async () => {
		...
		if (currentUser) {
			...
			if (!data.Javascript['7:4']) {
				await updateDoc(progressRef, {
					'Javascript.7:4': 'complete',
		...
		<div className='flex flex-col font-ubuntu bg-midnight text-white'>
			<div className='justify-items-center ml-20 mr-20 pl-20 pr-20'>
				<div className='flex justify-center space-x-8 p-5 m-5'>
					...
					<Link to='/javascript/7.5' onClick={setProgress}>
						<img
							className='bg-slate-400 hover:bg-gray-300 p-2'
							src={rightArrowIcon}
				...
				<p className='font-mono my-8'>
					The `Object.values()` method returns an array containing all the
				...
				<div className='editor-container'>
					<Editor
						...
						language='javascript'
						theme='vs-dark'
						value={Chapter1.topic4.example1}
				...
				<div className='mt-10'>
					...
					<p className='font-mono my-4'>
						Create an object called `drink` with properties `name`, `type`, and
						`calories`. Use `Object.values()` to get an array of the property
						values.
					</p>
				...
				<div className='mt-10'>
					...
					<p className='font-mono my-4'>
						Define an object `country` with properties `name`, `population`, and
						`language`. Use `Object.values()` to retrieve an array of the
						property values.
					</p>
				...
				<div className='mt-10'>
					...
					<p className='font-mono my-4'>
						Create an object `movie` with properties `title`, `director`, and
						`year`. Get an array of the property values using `Object.values()`.
					</p>
...
export default ObjectMethodValues;
