    ...
    const handleSubmit1 = async () => {
        ...
        try {
            const response = await fetch('https://syntactic-backend-d1b6d0af8db5.herokuapp.com/api/submitcode', {
              ...
              body: JSON.stringify({ code: code1, practice: Chapter1.topic3.practice1 }),
            });
        ...
        if (currentUser) {
            ...
            if (docSnap.exists()) {
                updateDoc(docRef, {
                    feedbacks: arrayUnion({ feedback: feedback, course: 'Python', problem:  Chapter1.topic3.practice1})
                });
            } else {
                
                await setDoc(docRef, {
                    feedbacks: [{course: 'Python', feedback: feedback, problem: Chapter1.topic3.practice1}]
                });
            }
        };
    ...
    const handleSubmit2 = async () => {
        ...
        try {
            const response = await fetch('https://syntactic-backend-d1b6d0af8db5.herokuapp.com/api/submitcode', {
              ...
              body: JSON.stringify({ code: code2, practice: Chapter1.topic3.practice2 }),
            });
        ...
        if (currentUser) {
            ...
            if (docSnap.exists()) {
                updateDoc(docRef, {
                    feedbacks: arrayUnion({ feedback: feedback, course: 'Python', problem:  Chapter1.topic3.practice2})
                });
            } else {
                
                await setDoc(docRef, {
                    feedbacks: [{course: 'Python', feedback: feedback, problem: Chapter1.topic3.practice2}]
                });
            }
    ...
    const handleSubmit3 = async () => {
        ...
        try {
            const response = await fetch('https://syntactic-backend-d1b6d0af8db5.herokuapp.com/api/submitcode', {
              ...
              body: JSON.stringify({ code: code1, practice: Chapter1.topic3.practice3}),
            });
        ...
        if (currentUser) {
            ...
            if (docSnap.exists()) {
                updateDoc(docRef, {
                    feedbacks: arrayUnion({ feedback: feedback, course: 'Python', problem:  Chapter1.topic3.practice3})
                });
            } else {
                
                await setDoc(docRef, {
                    feedbacks: [{course: 'Python', feedback: feedback, problem: Chapter1.topic3.practice3}]
                });
            }
		...
		<div className='flex flex-col font-ubuntu bg-midnight text-white'>
			<div className='justify-items-center ml-20 mr-20 pl-20 pr-20'>
				<div className='flex justify-center space-x-8 p-5 m-5'>
					...
					<Link to='/python/8.4' onClick={setProgress}>
						<img
							className='bg-slate-400 hover:bg-gray-300 p-2'
							src={rightArrowIcon}
				...
				<h1 className='text-2xl md:text-4xl font-bold my-4 bg-slate-700 p-4'>
					Encapsulation and Practical Applications
				</h1>

				<p className='font-mono my-8'>
					In this section, we'll explore the concept of encapsulation, which
					involves restricting access to certain parts of an object, and
					practical applications of classes
				</p>

				<p className='font-mono my-8'>Encapsulation</p>

				<p className='font-mono my-8'>
					Encapsulation is the concept of restricting access to certain parts of
					an object. In Python, you can achieve encapsulation by using private
					attributes and methods, typically indicated with a double underscore
					(__).
				</p>
				...
				<p className='font-mono my-8'>
					The Circle class uses a private attribute __radius to store the radius
					of the circle, preventing direct access from outside the class.
				</p>

				<div className='mt-10'>
					<h2 className='text-base md:text-xl font-bold bg-slate-700 p-2'>
						Practice 1: Book Class{' '}
					</h2>
					<p className='font-mono my-4'>
						Implement a class Book with private attributes __title and __author.
						Add methods to get and set these attributes. Test the getter and
						setter methods with different book details.
					</p>
				...
				<div className='mt-10'>
					...
					<p className='font-mono my-4'>
						Create a class BankAccount with private attributes __balance and
						__account_number. Implement methods for depositing and withdrawing
						money from the account, ensuring that the balance is properly
						encapsulated. Test these methods with various transactions.
					</p>
				...
				<p className='font-mono my-8'>Practical Applications</p>

				<p className='font-mono my-8'>
					Classes are used to model real-world objects and their behavior. They
					provide a structured way to represent and manipulate data.
				</p>
				...
				<p className='font-mono my-8'>
					The Car class represents a car's attributes and has a method to
					display car information.
				</p>

				<div className='mt-10'>
					<h2 className='text-base md:text-xl font-bold bg-slate-700 p-2'>
						Practice 3: Product Class
					</h2>
					<p className='font-mono my-4'>
						Define a class Product with attributes name, price, and quantity.
						Implement a method to calculate the total cost of a certain quantity
						of the product. Create an instance and test the method with
