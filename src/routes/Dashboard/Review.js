		...
		if (currentUser) {
			...
			const fetchReviewData = async () => {
				try {
					...
					if (docSnap.exists()) {
						console.log('Document Data (review):', docSnap.data());
	...
	const handleSubmit = async () => {
		...
		const response2 = await openai.chat.completions.create({
			messages: [
				{
					role: 'assistant',
					content: `The code bellow is an attempt to solve the given practice problem bellow. Based on this attempt determine if the code correctly solve the practice problem. if the code is correct return "Correct" else return "Incorrect". code is ${code}, practice problem is ${practice.practice}`,
				},
			],
			model: 'gpt-3.5-turbo',
			max_tokens: 5,
			...
			<div className='flex flex-wrap md:flex-nowrap  font-ubuntu bg-midnight text-white'>
				<div className='w-1/2 h-fit overflow-auto pl-5 pr-5'>
					...
					<p className='font-mono my-8'>
						Answer the practive questions bellow to the best of your ability.
						Your performance here will be used to build your specialized review
						list.
					</p>
